...
class BOX_MODEL(object):
    def __init__(self):
        ...
        #self.model = load_model(self.model, "/home/hisiter/working/text_mutex/extract_text_from_image/weights/model_235_full_v2.pth")
        # self.device = torch.device("cuda:1")
        # self.model.cuda()
        self.model.eval()

    def predict_box(self, img, have_cmnd):
        ...
        # show_result(img, list_box_optim)
        img_res = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
        img_res = np.ascontiguousarray(img_res)
        # box - [xmin, ymin, xmax, ymax]
        for box in list_box_optim:
            cv2.rectangle(img_res, (int(box[0]), int(box[1])), (int(box[2]), int(box[3])), (29, 187, 255), 2, 2)
        ...
        for cum in dict_cum_sorted:
            ...
            for box in cum:
                ...
                list_img.append(
                    img.copy().crop((max(0, xmin - int((xmax - xmin) * w_extend_size)), max(0, ymin - int((ymax - ymin) * h_extend_size)),
                                     xmax + int((xmax - xmin) * w_extend_size), ymax + int((ymax - ymin) * h_extend_size))))
            result_list_img_cum.append(list_img)

        return result_list_img_cum, img_res, dict_cum_sorted

    def sort_line_cmnd(self, boxes):

        if len(boxes) == 0:
            return []
        boxes = sorted(boxes, key=lambda x: x[1])   # y
        lines = [[]]

        # y_center = (boxes[0][1] + boxes[0][3]) / 2.0
        y_max_base = boxes[0][3]
        i = 0
        for box in boxes:
